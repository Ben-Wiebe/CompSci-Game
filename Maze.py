...
def generate(width, height, doTimer=False, difficulty=True):
    ...
    while frontiers:                            # Keep iterating as long as there are more frontiers
        ...
        if mapList[x][y] == 1:
            ...
            mapList[i][j] = 0    # If the tile is a wall, set the 2 tiles to be floor
...
if __name__ == '__main__': print("\n".join(" ".join(chr(9608) * 3 if y == 1 else "{:^3}".format(str(y)) for y in x) for x in generate(39, 39, doTimer=True, difficulty=True)))
