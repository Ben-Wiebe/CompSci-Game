...
def generate(width, height, doTimer=False):
    ...
    while frontiers:                            # Keep iterating as long as there are more frontiers
        ...
        if mapList[x][y] == 1: mapList[x][y] = new[2] + 1; mapList[i][j] = 0    # If the tile is a wall, set the 2 tiles to be floor
...
if __name__ == '__main__': print("\n".join(" ".join(" # " if y == 1 else "{:^3}".format(str(y)) for y in x) for x in generate(39, 39, doTimer=True)))
